      ...
      <div className="relative flex justify-between px-2">
        ...
        <div 
          className={`absolute top-5 left-0 h-1 -translate-y-1/2 z-0 rounded-full transition-all duration-700 ease-in-out ${
            isDelivered ? 'bg-emerald-500' : 'bg-brand-primary'
          }`}
        ...
        {STEPS.map((step, index) => {
          ...
          const isFinalStepAndDone = isActive && isDelivered;
          ...
          return (
            <div key={step.status} className="relative z-10 flex flex-col items-center gap-3">
              <div className={`w-10 h-10 rounded-full flex items-center justify-center transition-all duration-500 ${
                ...
                isFinalStepAndDone ? 'bg-emerald-500 text-white scale-110 shadow-lg shadow-emerald-500/20' :
              ...
              <div className="flex flex-col items-center">
                <span className={`text-[10px] font-black uppercase tracking-tight ${
                  isFinalStepAndDone ? 'text-emerald-500' :
                  isActive ? 'text-brand-primary' : 'text-slate-400'
                }`}>
                  {step.label}
                </span>
                {/* Only show the pinging pulse if active but NOT fully delivered */}
                {isActive && !isFinalStepAndDone && (
